...
newtype KVarSubst b v = KSu [(v, ExprBV b v)]
  deriving (Eq, Ord, Data, Typeable, Generic, Functor, Foldable, Traversable)

fromKVarSubst :: Hashable v => KVarSubst b v -> M.HashMap v (ExprBV b v)
fromKVarSubst (KSu su) = M.fromList su

toKVarSubst :: M.HashMap v (ExprBV b v) -> KVarSubst b v
...
instance (Ord v, Hashable v, Fixpoint v, Ord b, Fixpoint b) => Show (KVarSubst b v) where
  show = showFix

instance (Ord v, Hashable v, Fixpoint v, Ord b, Fixpoint b) => Fixpoint (KVarSubst b v) where
  toFix = toFix . fromKVarSubst

instance (Ord v, Hashable v, Fixpoint v, Ord b, Fixpoint b) => PPrint (KVarSubst b v) where
  pprintTidy _ = toFix
...
data ExprBV b v
          ...
          | PGrad  !KVar !(KVarSubst b v) !GradInfo !(ExprBV b v)
...
instance (Ord b, Fixpoint b, Ord v, Hashable v, Fixpoint v) => Fixpoint (ExprBV b v) where
...
instance (Ord b, Fixpoint b, Ord v, Hashable v, Fixpoint v, PPrint v) => PPrint (ExprBV b v) where
...
pprintQuant
  :: (Ord b, Fixpoint b, Ord v, Hashable v, Fixpoint v, PPrint v)
...
pprintReft :: (PPrint v, Ord v, Hashable v, Fixpoint v) => Tidy -> ReftV v -> Doc
...
pAnd, pOr     :: (Fixpoint b, Ord b, Fixpoint v, Ord v, Hashable v) => ListNE (ExprBV b v) -> ExprBV b v
pAnd          = simplify . PAnd
...
pIte :: (Fixpoint b, Ord b, Fixpoint v, Ord v, Hashable v) => ExprBV b v -> ExprBV b v -> ExprBV b v -> ExprBV b v
...
-- | Gradual Type Manipulation  ----------------------------
------------------------------------------------------------
pGAnds :: (Fixpoint b, Ord b, Fixpoint v, Ord v, Hashable v) => [ExprBV b v] -> ExprBV b v
pGAnds = foldl' pGAnd PTrue

pGAnd :: (Fixpoint b, Ord b, Fixpoint v, Ord v, Hashable v) => ExprBV b v -> ExprBV b v -> ExprBV b v
